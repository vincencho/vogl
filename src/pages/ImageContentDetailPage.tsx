  ...
  useEffect(() => {
    const fetchContent = async () => {
      ...
      const mockContent: Content = {
        ...
        description: "A stunning white halter top paired with high-waisted wide-leg pants. Perfect for a sophisticated, minimalist summer look.",
        ...
        <div className={`${viewportSize === 'desktop' ? 'w-1/2' : 'w-full'}`}>
          <div className="bg-white sticky top-0 z-10">
            <div className="flex justify-between border-t border-b">
              <button
                className={`px-4 py-2 ${activeTab === 'original' ? 'border-b-2 border-indigo-600' : ''}`}
                onClick={() => setActiveTab('original')}
              >
                제품
              </button>
              <button
                className={`px-4 py-2 ${activeTab === 'product' ? 'border-b-2 border-indigo-600' : ''}`}
                onClick={() => setActiveTab('product')}
              >
                코디
              </button>
              <button
                className={`px-4 py-2 ${activeTab === 'coordinate' ? 'border-b-2 border-indigo-600' : ''}`}
                onClick={() => setActiveTab('coordinate')}
              >
                무드
              </button>
              <button
                className={`px-4 py-2 ${activeTab === 'mood' ? 'border-b-2 border-indigo-600' : ''}`}
